...
+ **Years 1-5:** Rule-based algorithm (v1) handles student placements.
+ + **End of each year:** Satisfaction feedback is collected from matched students.
  + + **After 5 years:** A Random Forest model is trained on accumulated data.
    + + **Year 6+:** AI-powered matching (v2) takes over with a hybrid scoring approach.
                ...
                + + Hybrid mode: `alpha * AI_score + (1-alpha) * rule_score` for gradual transition
                  + + Feature importance analysis for interpretable results
                    ...
                    + ```
                      ...
                      ### v2 GUI Usage Steps
                      + **Load Room CSV** - Select the CSV file containing room data
                      + + **Load Student CSV** - Select new student data
                        + + **Generate/Load Feedback** - Generate synthetic data or load existing feedback
                          + + **Train Model** - Train the Random Forest model
                            + + **v2 Match** - Run AI-powered matching (adjust AI weight with alpha slider)
